...
- `--harness` scopes across providers. For normal commands, cq infers `--harness codex` in a Codex runtime and `--harness claude` everywhere else; `--all` disables that inference. `--source` remains a Claude-only dimension and cannot be combined with `--harness`.
