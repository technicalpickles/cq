...
When you change behavior, the matching docs need to move with it. `docs/cli-ux-conventions.md` has a "Keeping docs in sync" table mapping change types to the docs they affect (flags → README table, modules → AGENTS.md tree, sync/cache → Key patterns, etc.). Run through the matching row before considering a change done.
...
## Recording intent with lore

This project tracks intent, not just code, using [lore](https://lorevcs.com) (`.lore/`, gitignored like `.git/`). Each turn, record what was asked and the decision made:
...
Record durable decisions — rules for how cq should work going forward — not one-off asks, bug reports, or typos. It's cheap; do it often. Never record secrets. Commit related intent once a unit of work lands:

```
lore commit -m "short summary"
```

See `docs/lore.md` for the full reference (branching, remotes, `lore materialize`, the session-history backfill script).
