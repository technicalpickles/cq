...
- **Persistent cache + incremental sync.** `cache.rs` opens the cache DB and handles schema versioning. `indexer.rs` walks files, checks mtime + size against `file_registry`, and only re-parses what changed. `fs2` file locking serializes concurrent writers; readers fall back to cached data when the lock is busy. The scan recurses into `<session>/subagents/**` (excluding `journal.jsonl`); subagent rows carry the parent `session_id` plus `is_sidechain`/`agent_id`/`agent_type`/`workflow_id` tags. The Auto mtime fast-path is a recursive max so new deep files are detected. `read_json(..., ignore_errors=true)` tolerates a line it can't parse (e.g. an unpaired UTF-16 surrogate escape) by emitting that row as NULL rather than raising or dropping it; since `raw_records.json` is NOT NULL, `index_files` stages parsed rows in a temp table, drops the NULL ones (warning to stderr with a count and the file path), and inserts the rest.
